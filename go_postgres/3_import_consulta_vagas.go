...
func vagasProcessFile(db *sql.DB, path string) error {
	...
	flush := func() error {
		...
		if err := vagasExecuteBatch(db, insertHead, onConflict, placeholder, batch); err != nil {
			return err
		}
		batch = batch[:0]
		return nil
...
func vagasExecuteBatch(db *sql.DB, insertHead, onConflict, placeholder string, batch [][]interface{}) error {
	if len(batch) == 0 {
		return nil
	}
