use anyhow::Result;
use eleicoes_etl::common::{load_dotenv_quiet, mysql_pool, run_csv_import, CsvImportParams};
...
fn main() -> Result<()> {
    ...
    let pool = mysql_pool(MAX_WORKERS + 2)?;
    let _ = pool.get_conn()?; // ping implícito ao obter conexão
